...
GitHub Actions run daily cron jobs for email delivery, monitored through Sentry and tracked in PostHog. The analytics and error logs get checked daily. That data informs what gets built next.

## tl;dr

Checkout, calculation engine, synthesis layer, validator, PDF renderer, verification pipeline, email system, retry queue, blog, free tools, calendar, gift cards, reviews, and a Supabase database. All on a single Railway deployment.

Each piece got figured out along the way. The Human Design port took a couple of weeks. The verification pipeline was born out of shipping wrong data and the determination to never do it again.

Claude Code and I built this together, and I maintain it every day. That's the job.
