...
Then three calculation engines run independently. Western Tropical Astrology uses Kerykeion, which sits on top of Swiss Ephemeris, the same ephemeris NASA cross-references. Human Design uses an in-house port of HDKit that I rebuilt from scratch in Python because I didn't want to depend on someone else's API for a core calculation. That port alone took weeks: understanding the I Ching gate system, the hexagram-to-line mapping, how planetary positions convert to gate activations, and how channels form between defined centers. Numerology is Pythagorean, which is computationally simpler but still has to be precise.
